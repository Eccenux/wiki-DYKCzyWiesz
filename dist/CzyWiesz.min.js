...
(function() {
	...
	5: [ function(require, module, exports) {
		...
		`);
				...
				var $rules_paragraph = $('<p id="CzyWieszRules"></p>').html(`<small>Reguły: Zgłaszaj hasła, które powstały lub zostały rozbudowane nie dawniej niż <xd>10</xd> dni temu.
				Hasła muszą posiadać źródła w formie przypisów oraz muszą zawierać co najmniej <xkb>1</xkb> kB samej treści.</small>`).css({
				...
				const $main_table = $('<table></table>').css('width', '100%').append($ref_row).append($images_row).append($file_row).append($author_row).append($signature_row).append($wikiproject_row).append($events_row);
				const $dialog = $('<div id="CzyWieszGadget"></div>').append($title_paragraph).append($question_paragraph).append($question_textarea_paragraph).append($main_table).append($comment_paragraph).append($comment_textarea_paragraph).append($rules_paragraph).append($loading_bar);
				var buttons = {
					"Zgłoś": function() {
						if (D.sourced) {
							D.main.checkForm();
	...
	15: [ function(require, module, exports) {
		var config = {
			...
			refSectionsArr: [ 'Bibliografia', 'Przypisy' ],
			wikiConfigKey: 'dyk-extra-options',
			wikiConfigTitles: {
				"Wikiprojekt:Czy_wiesz/konfiguracja/opcje.json": "options",
				"Wikiprojekt:Czy_wiesz/konfiguracja/akcje.json": "events"
