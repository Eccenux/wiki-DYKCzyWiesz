...
(function() {
	...
	1: [ function(require, module, exports) {
		...
		function createDyk(DYKnomination) {
			...
			DYKnomination.errors = new ErrorInfo(arg1 => {
				DYKnomination.emailauthor(arg1);
			}, config.supportUser);
		}
		...
		module.exports = {
			DYKnomination: DYKnomination,
			createDyk: createDyk,
			createFullDyk: createFullDyk
		};
	}, {
		"./DykMain": 6,
		"./ErrorInfo": 8,
		...
		class DoneDialog {
			...
			init() {
				...
				DoneDialogInternal.prototype.initialize = function() {
					...
					this.content.$element.append(`<div class="warnings" style="display:none"><strong>Ostrzeżenia:</strong><ul></ul></div>`);
	...
	3: [ function(require, module, exports) {
		...
		`;
				if (await stdConfirm(confirmInfo)) {
					...
					if (!subpageTitle.length) {
						console.error('subpageTitle failed', {
							isSubpage: isSubpage,
							item: item
						});
						dd.update(`
				...
				`, true);
					...
					<ol>
						<li>Usuń zgłoszenie <a href="${mw.util.getUrl(D.getBaseNew(), {
							action: 'edit'
						})}" class="czywiesz-external" target="_blank">z listy propozycji</a>.
						<li>Dodaj zgłoszenie <a href="${mw.util.getUrl(D.getBaseDone(), {
							action: 'edit'
						})}" class="czywiesz-external" target="_blank">do listy ocenionych</a>.
		...
		`;
				...
				`, true);
					...
					dd.update(`
				<p>✅ Cofnięcie udane. <a href="${mw.util.getUrl(subpageTitle, {
						action: 'edit'
					})}">Dodaj powód otwarcia zgłoszenia</a> (możesz też ustawić status na „problemy”).</p>
			...
			async markUnDone(subpageTitle, summaryDone) {
				...
				wiki = wiki.replace(/\{\{(Załatwione|Zrobione)\}\}/gi, '{{s|$1}}');
				await apiAsync({
					url: '/w/api.php',
					type: 'POST',
					data: {
	...
	5: [ function(require, module, exports) {
		const {
			RevisionList
		} = require("./RevisionList");
		const {
			DykConfigExtra
		} = require("./DykConfigExtra");
		...
		`, $('#mw-content-text'));
				...
				for (var i = 0; i < REFS.ar2.length; i++) {
					if (REFS.ar1.match('#' + REFS.ar2[i] + '#')) {
						D.sourced = true;
						break;
					}
				}
				var SIGNATURE = D.wgUserName ? {
					name: D.wgUserName,
					disabled: ' disabled'
				} : {
					name: '~~' + '~',
				...
				var $title_paragraph = $('<p></p>').html('Tytuł artykułu: &nbsp;&nbsp;<input type="text" id="CzyWieszTitle" name="CzyWieszTitle" value="' + D.wgTitle + '" style="width: 476px;" disabled>');
		...
		`);
				var $question_textarea_paragraph = $('<p></p>').html(`
				...
				></textarea>
			`);
				var $ref_row = $('<tr id="CzyWieszRefs"></tr>').html('<td>Źródła: </td>' + '<td>' + (D.sourced ? D.config.yes : REFS.warn) + '</td>');
				if (D.sourced) {
					$ref_row.css({
						display: 'none'
					});
				}
				var $images_row = $('<tr></tr>').html('<td>Liczba grafik w artykule: </td>' + '<td><input type="number" min="0" id="CzyWieszImages" name="CzyWieszImages" value="' + IMAGES + '"' + 'style="width: 3.5em;">' + '<span id="CzyWieszGalleryToggler" style="display: none;"> &nbsp;<small><a href="#" role="button">(zaproponuj grafikę z artykułu)</a></small></span>');
				var $file_row = $('<tr></tr>').html('<td style="width: 30%;"><input type="checkbox" id="CzyWieszFile1" name="CzyWieszFile1" style="vertical-align: middle;"><label for="CzyWieszFile1"> Zaproponuj grafikę: </label></td>' + '<td><tt>[[Plik:</tt><input type="text" id="CzyWieszFile2" name="CzyWieszFile2" style="width: 52%; vertical-align: middle;" disabled><tt>|100px|right]]</tt></td>');
				var $author_row = $(`
		...
		`.replace(/\n\t+/g, '').trim());
				var $signature_row = $('<tr></tr>').html('<td>Twój podpis: </td>' + '<td><input type="text" id="CzyWieszSignature" name="CzyWieszSignature" value="' + SIGNATURE.name + '" style="width: 50%;margin-left: 2px;"' + SIGNATURE.disabled + '></td>');
				var $wikiproject_row = $('<span id="CzyWieszWikiprojectContainer"><small>(trwa ładowanie…)</small></span>');
				$wikiproject_row = $('<td></td>').append($wikiproject_row).append('<a id="CzyWieszWikiprojectAdd">(+)</a>');
				$wikiproject_row = $('<tr></tr>').append('<td>Powiadom wikiprojekt(y): </td>').append($wikiproject_row);
				var $comment_paragraph_checkbox = $('<input type="checkbox" id="CzyWieszCommentCheckbox" name="CzyWieszCommentCheckbox" style="vertical-align: middle;">').click(function() {
				...
				var $comment_textarea_paragraph = $('<p id="CzyWieszCommentContainer" style="display: none;"></p>').html('<textarea id="CzyWieszComment" style="width: 570px;" rows="2" value=""></textarea>');
				var $rules_paragraph = $('<p id="CzyWieszRules"></p>').html(`<small>Reguły: Zgłaszaj hasła, które powstały lub zostały rozbudowane nie dawniej niż 10 dni temu.
				Hasła muszą posiadać źródła (najlepiej w formie przypisów) oraz muszą zawierać co najmniej 2 kB samej treści.</small>`).css({
					border: '1px solid #F0F080',
					backgroundColor: '#FFFFE0',
					paddingLeft: '5px'
				});
				var $loading_bar = $('<div id="CzyWieszLoaderBar"></div>').css({
					width: '100%',
					backgroundColor: 'rgb(220, 220, 220)',
					border: '1px solid rgb(187, 187, 187)',
					borderRadius: '3px',
					boxSizing: 'border-box'
				}).html('<p id="CzyWieszLoaderBarParagraph" style="margin: 0 0 0 7px; position: absolute;">&nbsp;</p>' + '<div id="CzyWieszLoaderBarInner" style="width: 0; height: 20px; background-color: #ABEC46; border: none; border-radius: 3px;"></div>');
				var $dialog = $('<table></table>').css('width', '100%').append($ref_row).append($images_row).append($file_row).append($author_row).append($signature_row).append($wikiproject_row);
				$dialog = $('<div id="CzyWieszGadget"></div>').append($title_paragraph).append($question_paragraph).append($question_textarea_paragraph).append($dialog).append($comment_paragraph).append($comment_textarea_paragraph).append($rules_paragraph).append($loading_bar);
				var buttons = {
					"Zgłoś": function() {
						if (D.sourced) {
							D.main.checkForm();
				...
				if (IMAGES > 0) {
					...
					$('#CzyWieszGalleryToggler a').click(function() {
						var GALLERY = '<div id="CzyWieszGalleryHolder">' + `<div id="CzyWieszGallery">`;
			...
			async pagerevs() {
				...
				const {
					revisions,
					records
				} = await this.revisionList.readRevs(D.wgTitle, checkDays);
				...
				if (records.length > 0) {
					let {
						record: winner,
						size
					} = this.revisionList.findWinner(records, bigEdit);
				...
				D.articlesize = {
					...
					warn: editSize >= bigEdit ? '' : D.config.no + '&nbsp;&nbsp;<strong style="color: red;">Rozmiar ' + editSize + ' b dyskwalifikuje artykuł ze zgłoszenia!</strong> \x3c!--small>(<a class="czywiesz-external">info</a>)</small--\x3e'
				};
				...
				<th>Autor(ka)</th>
					...
					infoTable += `</table>`;
					const historyHref = mw.util.getUrl(null, {
						action: 'history'
					});
			...
			`;
				...
				$('<tr id="CzyWieszSize"></tr>').insertAfter($('#CzyWieszRefs')).html('<td>Rozmiar (>2 kb): </td>' + '<td>' + (D.articlesize.enough ? D.config.yes : D.articlesize.warn) + '</td>').css(D.articlesize.enough ? {
					display: 'none'
				} : {});
		...
		| 4. sprawdzenie = 
			...
			async inform_wLoop(sectionTitle_w, summary_w_newsection, summary_w, curWikiproject) {
				...
				if (!debug) {
					...
					data = data.replace(D.config.dykSectionIndicator, D.config.dykSectionIndicator + '\n' + infoTpl);
	...
	8: [ function(require, module, exports) {
		class ErrorInfo {
			...
			show() {
				...
				let dialog = $('<div id="CzyWieszErrorDialog"></div>').append(list).append($(`
	...
	9: [ function(require, module, exports) {
		class Loadbar {
			...
			next(task) {
				...
				if (task != 'error') {
					$('#CzyWieszLoaderBarInner').css({
						width: 100 * task / tasks + '%'
					});
				} else {
					$('#CzyWieszLoaderBarInner').css({
						backgroundColor: 'red'
	...
	10: [ function(require, module, exports) {
		class ReadJsonCached {
			...
			isCacheValid() {
				...
				let cacheAge = (Date.now() - this.cacheTimestamp) / (1e3 * 60 * 60);
	...
	11: [ function(require, module, exports) {
		class RevisionList {
			...
			daysAgo(isoDate, today = new Date()) {
				...
				const differenceInDays = Math.floor(differenceInTime / (1e3 * 60 * 60 * 24));
	...
	12: [ function(require, module, exports) {
		class Wikiprojects {
			...
			load() {
				gadget.getWikiprojects().then(data => {
					const list = data.wikiprojects.map(w => ({
						name: w.name,
						page: w.page
					}));
					this.list = list;
	...
	13: [ function(require, module, exports) {
		...
		function apiAsync(call) {
			return new Promise((resolve, reject) => {
				apiAjax(call).done(function(data) {
					resolve(data);
	...
	15: [ function(require, module, exports) {
		var config = {
			...
			dykSectionIndicator: '\x3c!-- Nowe zgłoszenia CzyWiesza wstawiaj poniżej tej linii. Nie zmieniaj i nie usuwaj tej linii --\x3e',
		...
		if (namespaceNumber === 0) {
			...
			mw.loader.using([ "mediawiki.util" ]).then(function() {
				$(document).ready(function() {
					const link = mw.util.addPortletLink('p-tb', '#', (window.DYKnomination_is_beta === true ? 'BETA: ' : '') + DYKnomination.config.portlet_title, 't-DYKnomination');
					$(link).click(e => {
						e.preventDefault();
